...
bin.sf <- function(df, nbins, minpts, nvals.min = 5, verbose = F, tol = 10) {
  if (is.null(nbins) || floor(minpts) != minpts) {
    msg <- paste(
      "bins.sf: parametros inconsistentes",
      "\nNumero de bines nulo?",
      is.null(nbins),
      "\nCasos minimos x bin es entero?",
      minpts
    )
  ...
  if (nvals < nvals.min) {
    msg <- paste(
      "Valores unicos insuficientes!",
      "Se esperaban",
      nvals.min,
      "valores únicos y se obtuvo",
      nvals,
      "Lista de valores: ",
      vals |> paste(collapse = ", ")
    )
  ...
  for (i in 1:nvals) {
    ...
    # Antes trataba de parecerme al acumulado     if (aw >= binaw[k] || i >= nvals) {
    if (w >= minbinw) {
      # Cierro el bin[k]
      binhi[k] <- df[[i, "x"]]
      binq[k] <- q
  ...
  if (bins_ok$n < 2) {
    msg <- paste(
      "No se pudo partir en al menos dos bines con",
      round(100 / nbins, 1),
      "% de las observaciones y al menos",
      minpts,
      "observaciones (pesadas)"
    )
    return(list(error = T, msg = msg))
  }
  ...
  if (sum(df$w) != sum(bins$binw)) {
    msg <- paste("El peso total fue", sum(bins$binw), " y se esperaba ", sum(df$w))
    return(list(error = T, msg = msg))
  }
...
bin_monot <- function(
  ...
  if (res$error) {
    return(list(x = x, y = y, col_id = j, error = T, error_detalle = res$msg))
  }
  ...
  if (!is.null(c$error) | is.na(c$result)) {
    return(list(
      ...
      error = T,
      error_detalle = paste(
        "Correlacion de Spearman entre variable agrupada y objetivo nula o cero. ",
        c$error,
        c$warnings
      )
    ))
  ...
  if (abs(c$result) >= 0.95) {
    return(list(
      ...
      error = T,
      error_detalle = paste(
        "Correlacion de Spearman entre variable agrupada y objetivo mayor a 0.95! ",
        c$result
      )
    ))
  }
  ...
  df_nonulos |>
    group_by(bin) |>
    summarise(w = sum(.weight), yw = sum(y * .weight)) |>
    mutate(GR = yw / w, SGR = GR * sentido) |>
    arrange(bin) |>
  ...
  df |>
    filter(is.na(x)) |>
    bind_rows(res_antes$df) |>
    woe_table() -> woes_tab_nomonot

  df_orig |>
    apply_grouping_monot(
      woes_tab_nomonot,
      x,
      col_agrup,
      rampa = 0,
      woe_nulos = res$woe_nulos
    ) |>
    purrr::pluck("df") -> df
  ...
  if (res_agrup$error) {
    res$error <- TRUE
    res$error_detalle <- paste(
      "Hubo un problema en el cálculo de tab_iv_antes",
      res_agrup$msg
    )
    return(res)
  } else {
    tab_iv_antes <- res_agrup$tab
  }
  ...
  if (res_agrup$error) {
    res$error <- TRUE
    res$error_detalle <- paste(
      "Hubo un problema en el cálculo de tab_iv_stp",
      res_agrup$msg
    )
    return(res)
  } else {
    tab_iv_stp <- res_agrup$tab
  }
  ...
  if (res_agrup$error) {
    res$error <- TRUE
    res$error_detalle <- paste(
      "Hubo un problema en el cálculo de tab_iv_pwl",
      res_agrup$msg
    )
    return(res)
  } else {
    tab_iv_pwl <- res_agrup$tab
  }
  ...
  if (!res_check) {
    res$error <- TRUE
    res$error_detalle <- paste(
      "Fallo interno! woes_tab !=  tab_iv_stp",
  ...
  assertthat::are_equal(
    tab_iv_stp |>
      filter(col_agrup == 'Total') |>
      select(CntRec, CntBad, BadRate, WoE),
    tab_iv_pwl |>
      filter(col_agrup == 'Total') |>
      select(CntRec, CntBad, BadRate, WoE),
    tol = 0.05
  ...
  if (rampa == 1) {
    iv_despues <- iv_monot_cont
  } else {
    iv_despues <- iv_monot_discret
  }
  res <- append(
    res,
    list(
      tab_iv_stp = tab_iv_stp,
      tab_iv_pwl = tab_iv_pwl,
      ...
      iv_perd = if_else(
        iv_antes == 0,
        0,
        100 * (iv_antes - iv_despues) / iv_antes
      ),
      iv_perd_discret = if_else(
        iv_antes == 0,
        0,
        100 * (iv_antes - iv_monot_discret) / iv_antes
      ),
      iv_perd_cont = if_else(
        iv_antes == 0,
        0,
        100 * (iv_antes - iv_monot_cont) / iv_antes
      )
    )
  )
  if (verbose) {
    ...
    if (length(var.sel) > 0 && !is.na(var.sel)) {
      ...
      if (all(tab_coef$sign == 1)) {
        ...
        winner_stats <- tibble(
          Variable = var.sel,
          Df = 1L,
          Deviance = mod.step$deviance,
          AIC = mod.step$aic,
          LRT = vars.fwd |> filter(Variable == var.sel) |> pull(LRT),
          pval = vars.fwd |> filter(Variable == var.sel) |> pull(pval),
          Paso = idx
        )
