...
if (requireNamespace("future", quietly = TRUE)) {
  ...
  log_info("Parallel processing enabled with {n_cores} workers.")
}
```

```{r SetupForwardK-B}
# Setup progress handlers - no console output during rendering
# Progress is tracked via progress.json file instead
if (interactive()) {
  # Only show cli progress in interactive mode
  progressr::handlers(global = TRUE)
  progressr::handlers("cli")
} else {
  # In batch mode (rendering), use silent handler
  ...
  error = function(e) {
    if (any(train$.weight != 1)) {
      error_custom("El método ForwardK no convergió. Intentar con valores de par_weight constantes o constantes por cada valor de par_target.",
       ...
       # Enhanced error reporting - use paste to avoid glue interpretation issues
      error_msg <- paste("El método ForwardK no convergió por causas internas. Detalle:", e$message)
      error_custom(error_msg, ">"=cli::col_red("Cod 203"))
    }
    return(NULL)
  }
)
...
tab |>
  ...
  xlab("Paso") # Mantengo la vieja nomenclatura
```
...
repKS(tab.all, caption="Validación Modelo Logístico en All", totales = TRUE)
```

#### Perfomance Train

```{r PerformanceTrain}
repKS(tab.train, caption="Validación Modelo Logístico en Train", totales = TRUE)
```
...
} else repKS(tab.test, caption="Validación Modelo Logístico en Test In-Sample", totales = TRUE)
```
...
if (par_split > 1)
  repKS(tab.valid, caption="Validación Modelo Logístico en Valid In-Sample", totales = TRUE)
```
...
if (tab$Max_Dif_SQL_R > 1)
  log_warn("Hubo un problema en la verificación de Scores! (Cod: 201)")
```
...
```{r reportes_dev_noyc}
repKS(tab_all_sql, caption="Validación muestra de desarrollo en SQL", totales = TRUE)
```
...
tab |> tab_niv_default_fct() -> tab_niv_default
```
...
if (!par_quick) {
  c(
    file_list_move,
    "Estab_ivs_cand_InS.Rdat" |> with_working_path()
  ) -> file_list_move
}
# Mueve los archivos al zip.  No guarda los paths
f.name |> zip(flags = "-r9Xjm", files = file_list_move)
# Copia los archivos al zip.
f.name |> zip(flags = "-r9Xj", files = file_list_keep)
if (!fs::file_exists(f.name)) error_custom(
    "No se logró grabar el archivo {f.name}",
    ">" = cli::col_red("Cod 122")
  )
```
...
```{r End}
#| output: false
log_info("Fin del cuaderno de Modelado")
```
