...
if (data_source_type == 'DELIM') {
  rlang::try_fetch(
    ...
    vroom::vroom(delim = data_source_delim,
                 col_types = cols(.default = col_character()),
                 escape_double = FALSE,
                 trim_ws = TRUE, show_col_types = FALSE,
                 .name_repair = "universal") -> Datos_Modelo
  )

  Datos_Modelo |>
    map_lgl(function(col) {
    num_conv <- suppressWarnings(as.numeric(col))
    # If an original non-NA element becomes NA after conversion, then this column is not purely numeric.
    all(is.na(col) | !is.na(num_conv))
  }) -> is_col_numeric

  # Build a new column specification list: if a column can be numeric, use col_double(), otherwise col_character()
  Datos_Modelo |>
    names() |>
    map(function(colname) {
    if (is_col_numeric[[colname]]) {
      col_double()
    } else {
      col_character()
    }
  }) -> new_specs
  names(Datos_Modelo) -> names(new_specs)
  do.call(cols, new_specs) -> spec_source
...
if (!res$wt > 0)
  error_custom("Los pesos no pueden ser todos ceros!",
               ...
               ">"=cli::col_red("Cod 109"))
if (!res$wn)
  log_warn("No todos los pesos son enteros! Se esperan frecuencias en la variable {par_weight}. La convergencia de la regresión logística NO está garantizada! (Cod: 200)")
```
...
n_pred_cont <- length(pred.cont)
for (i_cont in seq_along(pred.cont)) {
  col <- pred.cont[i_cont]
  generada <- 0
  msg <- ""
  ...
  write_progress_json(task = "Binning_Cont", task_progress = i_cont / n_pred_cont,
                      task_detail = paste("Variable", i_cont, "of", n_pred_cont))
  ...
  if (res$error)
    ...
    if ("iv" %in% names(res)) {
      if (res$iv >= par_iv | col %in% cols_forzadas_a_predictoras) {
        ...
        if (res_g$alerta) {
          log_warn("Advertencia: {res_g$msg}")
          ...
          if (!(col %in% cols_forzadas_a_predictoras))
            log_info("Se discretizó la variable {col} IV: {round(res$iv,3)}>={par_iv}")
          else {
...
tab = list()
dis <- idx <- 1
n_pred_fac <- length(pred.fac)
for (i_fac in seq_along(pred.fac)) {
  col <- pred.fac[i_fac]
  generada <- 0
  msg <- ""
  res <- bin_factor(train, col, minpts=par_minpts_cat, Good="Good", maxlevels=par_maxlevels)
  write_progress_json(task = "Binning_Fact", task_progress = i_fac / n_pred_fac,
                      task_detail = paste("Variable", i_fac, "of", n_pred_fac))
  tab[[idx]] <- c(var=col, var_gen=paste0(col,"_g"), generada=generada, msg=msg, res)
  if (res$error)
    log_warn("Variable {col} descartada por error. {res$error_detalle}")
  else
    if ("iv" %in% names(res)) {
      if (res$iv > par_iv | col %in% cols_forzadas_a_predictoras) {
...
cols_generadas <- tab.bins |> keep(~ .x$generada==1) |> map_chr(~.x$var_gen)
if (empty_param(cols_generadas))
  error_custom("x"="No se generó ninguna variable candidata!",
      "i"="Relajar restricciones?",
      ">"=cli::col_red("Cod 211"))
...
train <- df |> select(all_of(cols_generadas), .part, .weight, Good,Bad) |>
...
```{r corr}
X.cor <- train |> select(all_of(cols_generadas)) |>
...
```{r End}
#| output: false
log_info("Fin del cuaderno de Limpieza y Transformación de Variables")
```
