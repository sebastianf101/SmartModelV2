...
print_help() {
    echo "Validación en Muestra de Desarrollo"
    ...
    echo "    'Control de SmartModelStudio.xlsx' (ó json), Modelo.zip y Muestra_Desarrollo.txt"
    ...
    echo "Ejecuta el Cuaderno de Validación en Muestra de Desarrollo."
...
pipeline_desa() {
    ...
    if [ $exit_status_1 -ne 0 ] || [ $exit_status_2 -ne 0 ] || [ $exit_status_3 -ne 0 ] \
      || [ $exit_status_4 -ne 0 ]; then
      echo "Error: Fallo en copia de datos de entrada!"
      return 1;
    fi

    echo
    echo "Inicio Cuaderno de Validación en Desarrollo"
    docker exec --user $BSM_USER --workdir $BSM_DIR \
      $BSM_CONTAINER_SERVICE quarto render "Cuadernos/Validation_InS.qmd" \
        --log "$BSM_DIR/Trabajo/results_valid_insample.log" #--log-format json-stream
    exit_status_1=$?
    docker cp $BSM_CONTAINER_SERVICE:$BSM_DIR/Trabajo/results_valid_insample.log "$OUTPUT_DIR"/
    exit_status_2=$?
    if [ $exit_status_1 -ne 0 ] || [ $exit_status_2 -ne 0 ]; then
      echo "Error: Cuaderno Validation_InS.qmd falló."
      return 1;
    fi
    echo "Fin Cuaderno de Validación en Desarrollo"
...
echo "*****************************************************************************"
echo "Fin de proceso de Validación en Muestra de Desarrollo sin errores detectados."
echo "*****************************************************************************"
